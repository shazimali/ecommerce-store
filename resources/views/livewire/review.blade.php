<div class="px-8 py-10 text-black dark:text-white">
    <div class="flex justify-center">
        <h3 class="text-2xl font-semibold py-2">My Reviews</h3>
    </div>
    <div class="flex justify-center mb-6 ">
        <button wire:click="$set('tab', 'toBeReviewed')"
            class="px-4 py-2 font-medium {{ $tab === 'toBeReviewed' ? 'text-primary border-b-2 border-primary' : 'text-gray-400' }}">
            To Be Reviewed
        </button>
        <button wire:click="$set('tab', 'history')"
            class="px-4 py-2 font-medium {{ $tab === 'history' ? 'text-primary border-b-2 border-primary' : 'text-gray-400' }}">
            History
        </button>
    </div>
    @if ($tab === 'toBeReviewed')
            ...
            <div class="grid grid-cols-2">
                <div class="grid grid-cols-[20%_80%] gap-5 border-b-2 py-2">
                    @foreach ($forms as $index => $item)
                        <img class="h-30 w-30 inline-block" src="{{ asset('storage/' . $item['image']) }}"
                            alt="{{ $item['image'] }}">
                        <div>
                            <div class="py-1 text-xs max-w-60"><b>{{ $item['title'] }}</b></div>
                            <form wire:submit.prevent="saveReview({{ $index }})">
                                <div>
                                    <div>
                                    ...
                                    <div class="flex items-center mt-2">
                                        @for ($i = 1; $i <= 5; $i++)
                                            <svg wire:click="setRating({{ $i }},{{ $index }})"
                                                class="w-8 h-8 cursor-pointer {{ $i <= ($item['rating'] ?? 0) ? 'text-primary' : 'text-gray-300' }}"
                                    ...
                                    <div>
                                        <button type="submit"
                                            class="font-semibold bg-primary text-white mt-1 w-full py-3 text-center mt-4">
                                            <svg wire:loading wire:target="saveReview({{ $index }})"
                                                aria-hidden="true" role="status"
                                                class="inline mr-1 w-6 h-6 text-white animate-spin"
    ...
    @if ($tab === 'history')
        ...
        @else
            @foreach ($history as $review)
                <div class="p-4 border-b grid grid-cols-[80px_auto] gap-4 items-start">
                    {{-- Product Image --}}
                    <div>
                        @if ($review->product && $review->product->image)
                            <img src="{{ asset('storage/' . $review->product->image) }}"
                                alt="{{ $review->product->title }}" class="w-30 h-30 inline-block">
                        @endif
                    </div>

                    {{-- Review Content --}}
                    <div>
                        <h3 class="font-bold">{{ $review->product->title ?? 'Unknown Product' }}</h3>
