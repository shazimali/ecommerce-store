    ...
    <div class="grid lg:grid-cols-[40%_60%] md:grid-cols-[40%_60%] sm:grid-cols-[40%_60%] xs:grid-cols-[100%] px-8 py-10">
        <div class="px-2">
            ...
            <div class="flex justify-center mt-2">
                ...
                @if ($media)
                    <img class="border border-solid h-16 w-16 mx-auto cursor-pointer" src="{{ $media }}" wire:click="changeActiveImage('{{ $media }}')"/>    
                @endif
                @endforeach
            </div>
       </div>
       <div>
            ...
            <div>
                ...
                <div class="inline-block">
                    ...
                    <span class="inline-block pr-2 cursor-pointer" 
            ...
            <div class="grid grid-cols-[25%_75%] py-2">
                <div class="flex lg:flex-col md:flex-col sm:flex-col lg:gap-1 md:gap-1 sm:gap-1">
                    <div @dblclick.prevent class="flex items-center">
                        <button wire:click="decrementQty({{ $qty-1 }})" class="flex h-10 items-center justify-center  px-4 py-2 bg-primary text-white" aria-label="subtract">
                        ...
                        <input wire:model="qty" value="{{ $qty }}" id="counterInput" type="text" class="border-x-none h-10 w-20 rounded-none border-y border-secondary bg-secondary text-center text-black" readonly />
                        <button wire:click="incrementQty({{ $qty+1 }})" class="flex h-10 items-center justify-center  px-4 py-2 text-white bg-primary" aria-label="add">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" aria-hidden="true" stroke="currentColor" fill="none" stroke-width="2" class="size-4">
                ...
                <div class="text-start">
                    <button wire:click="addToCart('{{ $product['slug'] }}')"  wire:loading.attr="disabled"  class="bg-primary text-white py-2 px-4 cursor-pointer w-1/3">
                        <i class="fa-solid fa-cart-shopping"></i>
                        <svg wire:loading wire:target="addToCart('{{ $product['slug'] }}')" aria-hidden="true" role="status" class="inline mr-1 w-4 h-4 text-primary animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
