<div class="drawer drawer-end">
    ...
    <div class="drawer-side text-black z-50">
        ...
        <div class="menu bg-white dark:bg-black min-h-full lg:w-96 md:w-96 sm:w-80 w-80 p-4 text-start flex flex-col justify-between">
            <div>
                <div class="flex justify-between items-center border-b border-b-secondary dark:border-b-slate-800 pb-2 mb-3">
                    ...
                    <label for="cart-side-bar" class="text-neutral-500 hover:text-black dark:hover:text-white cursor-pointer text-lg">
                ...
                @if($cart_count > 0)
                    <div class="overflow-y-auto max-h-[calc(100vh-250px)] divide-y divide-secondary dark:divide-slate-800 pr-1">
                        @foreach ($cartItems as $index => $crt)
                            <div wire:key="desktop-cart-item-{{ $crt['slug'] }}-{{ $crt['color'] }}-{{ $crt['is_bundle'] ?? 0 }}"
                                class="py-3 relative">
                                <button type="button"
