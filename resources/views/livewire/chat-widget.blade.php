<div class="fixed bottom-4 right-4 sm:bottom-5 sm:right-5 font-sans" style="z-index: 2147483647;">
    ...
    <button wire:click="toggleWidget"
        ...
        <!-- AI Spark Icon -->
        <svg class="w-6 h-6 text-white shrink-0" fill="none" stroke="currentColor" stroke-width="1.8"
            viewBox="0 0 24 24">
    ...
    @if($isOpen)
        <div x-data="{ showScrollBtn: false, isNearBottom: true }" x-init="$nextTick(() => {
            ...
            <div class="bg-primary p-4 text-white flex items-center justify-between shadow-md">
                <div class="flex items-center gap-3">
                    <div
                        class="w-9 h-9 rounded-full bg-white/25 flex items-center justify-center font-bold text-white shadow-inner">
                        ✨
                    </div>
                    <div>
                ...
                <div class="flex items-center gap-1">
                    <button wire:click="clearChat" title="Clear Chat"
                        class="p-1.5 hover:bg-white/20 rounded-lg transition-colors text-white/90 hover:text-white">
                    ...
                    <button wire:click="toggleWidget" title="Close"
                        class="p-1.5 hover:bg-white/20 rounded-lg transition-colors text-white/90 hover:text-white">
            ...
            <div x-ref="chatContainer"
                class="flex-1 p-4 overflow-y-auto space-y-3 bg-[#f8f9fe] dark:bg-zinc-950/80 text-xs">
                @foreach($messages as $msg)
                    <div class="flex flex-col {{ $msg['role'] === 'user' ? 'items-end' : 'items-start' }}">
                        <div
                            class="max-w-[88%] rounded-2xl px-3.5 py-2.5 shadow-sm leading-relaxed {{ $msg['role'] === 'user' ? 'bg-primary text-white font-medium rounded-br-none' : 'bg-white dark:bg-zinc-800 dark:text-zinc-100 text-zinc-800 border border-secondary dark:border-zinc-700/60 rounded-bl-none' }}">
                            @php
                ...
                <div wire:loading wire:target="sendMessage, sendQuickPrompt" class="flex items-start">
                    <div
                        class="max-w-[88%] rounded-2xl px-3.5 py-3 shadow-sm bg-white dark:bg-zinc-800 border border-secondary dark:border-zinc-700/60 rounded-bl-none">
            ...
            <div
                class="px-3 py-2 bg-white dark:bg-black border-t border-secondary dark:border-zinc-800 flex gap-1.5 overflow-x-auto text-[10px] no-scrollbar">
                <button wire:click="sendQuickPrompt('Track my order')"
                    class="whitespace-nowrap bg-secondary dark:bg-zinc-800 hover:bg-primary/20 text-zinc-800 dark:text-zinc-200 hover:text-primary border border-primary/20 px-2.5 py-1.5 rounded-full transition-colors font-medium">
                    📦 Track Order
                </button>
                <button wire:click="sendQuickPrompt('List all products')"
                    class="whitespace-nowrap bg-secondary dark:bg-zinc-800 hover:bg-primary/20 text-zinc-800 dark:text-zinc-200 hover:text-primary border border-primary/20 px-2.5 py-1.5 rounded-full transition-colors font-medium">
                    🛒 All Products
                </button>
                <button wire:click="sendQuickPrompt('What categories do you have?')"
                    class="whitespace-nowrap bg-secondary dark:bg-zinc-800 hover:bg-primary/20 text-zinc-800 dark:text-zinc-200 hover:text-primary border border-primary/20 px-2.5 py-1.5 rounded-full transition-colors font-medium">
                    📁 Categories
                </button>
                <button wire:click="sendQuickPrompt('What is your return and refund policy?')"
                    class="whitespace-nowrap bg-secondary dark:bg-zinc-800 hover:bg-primary/20 text-zinc-800 dark:text-zinc-200 hover:text-primary border border-primary/20 px-2.5 py-1.5 rounded-full transition-colors font-medium">
                    🔄 Returns
                </button>
                <button wire:click="sendQuickPrompt('Free shipping policy')"
                    class="whitespace-nowrap bg-secondary dark:bg-zinc-800 hover:bg-primary/20 text-zinc-800 dark:text-zinc-200 hover:text-primary border border-primary/20 px-2.5 py-1.5 rounded-full transition-colors font-medium">
                    🚚 Shipping
                </button>
                <button wire:click="sendQuickPrompt('Show your best selling products')"
                    class="whitespace-nowrap bg-secondary dark:bg-zinc-800 hover:bg-primary/20 text-zinc-800 dark:text-zinc-200 hover:text-primary border border-primary/20 px-2.5 py-1.5 rounded-full transition-colors font-medium">
                    ⭐ Best Sellers
                </button>
                <button wire:click="sendQuickPrompt('Terms and conditions')"
                    class="whitespace-nowrap bg-secondary dark:bg-zinc-800 hover:bg-primary/20 text-zinc-800 dark:text-zinc-200 hover:text-primary border border-primary/20 px-2.5 py-1.5 rounded-full transition-colors font-medium">
                    📋 Terms
                </button>
                <button wire:click="sendQuickPrompt('I need to talk to a human agent')"
                    class="whitespace-nowrap bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#1ea952] dark:text-emerald-400 border border-[#25D366]/30 px-2.5 py-1.5 rounded-full transition-colors font-medium">
                    💬 Talk to Human
                </button>
            </div>
            ...
            <form wire:submit.prevent="sendMessage"
                class="p-3 bg-white dark:bg-black border-t border-secondary dark:border-zinc-800 flex items-center gap-2">
                <input type="text" wire:model="inputMessage" wire:loading.attr="disabled"
                    wire:target="sendMessage, sendQuickPrompt" placeholder="Ask about products or order status..."
                ...
                <button type="submit" wire:loading.attr="disabled" wire:target="sendMessage, sendQuickPrompt"
                    ...
                    <!-- Send icon (default) -->
                    <svg wire:loading.remove wire:target="sendMessage, sendQuickPrompt" class="w-4 h-4 rotate-90 text-white"
                    ...
                    <svg wire:loading wire:target="sendMessage, sendQuickPrompt" class="w-4 h-4 animate-spin text-white"
