<div class="bg-white dark:bg-black"
            ...
            <div class="py-2">
                ...
                @else
                    <div>
                        <div class="flex items-baseline gap-4">
                            @if ($bundle->price_detail && $bundle->price_detail->discount > 0 && (Carbon\Carbon::today()->toDateString() >= $bundle->price_detail->discount_from && Carbon\Carbon::today()->toDateString() <= $bundle->price_detail->discount_to))
                                ...
                                <span class="text-lg text-neutral-400 line-through">
                                    {{ $bundle->price_detail->country->currency }}
                                    {{ number_format($bundle->price_detail->price, 2) }}
                                </span>
                                <span
                                    ...
                                    -{{ $bundle->price_detail->discount }}% off
                                </span>
                            @elseif ($bundle->price_detail)
                                <span class="text-3xl font-extrabold text-primary">
                                    {{ $bundle->price_detail->country->currency }}
                                    {{ number_format($bundle->price_detail->price, 2) }}
                                </span>
                        ...
                        @if ($bundle->price_detail && $bundle->price_detail->discount > 0 && (Carbon\Carbon::today()->toDateString() >= $bundle->price_detail->discount_from && Carbon\Carbon::today()->toDateString() <= $bundle->price_detail->discount_to))
                            @php
                                $daysLeft = Carbon\Carbon::today()->diffInDays(Carbon\Carbon::parse($bundle->price_detail->discount_to));
                            @endphp
            ...
            <div x-show="tab === 1" class="space-y-6">
                ...
                @else
                    <div class="text-center py-12 text-neutral-400 dark:text-neutral-500">
                        ...
                        <span>No reviews yet for this bundle.</span>
                    </div>
                @endif
            </div>
        </div>
    ...
    @if(count($relatedBundles) > 0)
        <div class="mt-16 text-center">
            <h2 class="font-bold lg:text-4xl md:text-4xl sm:text-2xl xs:text-2xl dark:text-secondary">Related Bundles</h2>
        </div>
            ...
            <div class="container xl:max-w-screen-xl 2xl:max-w-screen-2xl px-4 mx-auto sm:px-6">
                ...
                <div class="swiper related-products-swiper-container lg:px-6 md:px-6 sm:px-2 xs:px-2">
                    <div class="swiper-wrapper">
                        @foreach($relatedBundles as $pr)
                            <div class="swiper-slide bg-white dark:bg-black py-4">
                                @include('inc.bundle_box_trending')
                            </div>
                        @endforeach
                    </div>
                </div>
