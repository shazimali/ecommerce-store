...
<div class="grid md:grid-cols-3 lg:grid-cols-3 sm:grid-cols-2 xs:grid-cols-1 mt-16 px-8 gap-5">
    @foreach ($subCategories as $category)
        <a href="" class="border border-secondary dark:border-slate-800 p-5">
            <div x-show="{{ $category->product_heads->count() }}" class="text-gray-500 text-end">{{ $category->product_heads->count() }} Products</div>
